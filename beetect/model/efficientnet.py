...
class EfficientNet(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, model_name, local_state_dict=None, num_classes=1000, in_channels=3):
        model = cls.from_name(model_name, override_params={
                              'num_classes': num_classes})
        load_pretrained_weights(
            model, model_name, local_state_dict, load_fc=(num_classes == 1000))
        if in_channels != 3:
    ...
    @classmethod
    def from_pretrained(cls, model_name, local_state_dict=None, num_classes=1000):
        model = cls.from_name(model_name, override_params={
                              'num_classes': num_classes})
        load_pretrained_weights(
            model, model_name, local_state_dict, load_fc=(num_classes == 1000))
