...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test, total_batch_size, gs, opt,
                                       hyp=hyp, augment=False, cache=args.cache_images, rect=True, rank=-1,
                                       world_size=args.world_size, workers=args.workers)[0]  # only runs on process 0
...
if __name__ == '__main__':
    ...
    # altering classes # randomly re-initializes the output layers since the shape is no longer
    # the same as the pretrained ones
    ...
    if not args.evolve:
        tb_writer = None
        if args.global_rank in [-1, 0]:
            logger.info('Start Tensorboard with "tensorboard --logdir %s", view at http://localhost:6006/' % args.logdir)
